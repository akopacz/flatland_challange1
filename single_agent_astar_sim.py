...
for a_id in show_agents:
    ...
    astar_paths_readable[a_id] = [node.point for node in astar_planner.aStar(start, end)]
...
for step in range(step):
    ...
    for a_id in show_agents:
        ...
        if ag.status == RailAgentStatus.ACTIVE:
            if ag.position == astar_paths_readable[a_id][agent_current_node[a_id]+1]:
                # if entered a new cell decide action
                agent_current_node[a_id] += 1
                # follow path defined by a*
                # decide which way to go next
                from_ = astar_paths_readable[a_id][agent_current_node[a_id]]
                to = astar_paths_readable[a_id][agent_current_node[a_id] + 1]
