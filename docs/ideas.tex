...
\begin{tikzpicture}[remember picture,overlay,>=latex]%
  ...
  { \begin{minipage}{.75\linewidth}
    \textbf{Conflicts}\\
     When the agent is \emph{expected to reach} that position, it could be at a conflicting state.
    \vspace*{.3cm}
    \begin{alertblock}{Resolution}
      The $A^*$ algorithm will output a modified graph that includes times when a node
    ...
    \item To have a few \alert{priority classes} only: suggestion is to have:
      \begin{description}
